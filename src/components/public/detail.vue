	...
	<div class="index-container" style="background: #fff">
		...
		<div class="detail-content" v-show="detail && detailVisible" @scroll="detailScroll" ref="detailMain">
			...
			<div class="about-info" v-show="detail && detail.content && detailVisible && aboutInfoList && aboutInfoList.list.length">
				...
				<news-list :news-data="aboutInfoList" :detail-name="'recommend'" :detail-title="$route.query.title">
					<div slot="none"></div>
				</news-list>
			</div>
		</div>
		<div class="comment-box" v-show="commentInput && detail && detail.id">
			...
			<i class="iconfont" :class="{'icon-collection': isCollection, 'icon-collection_fill': !isCollection}" v-show="$route.name === 'detail'" @click="collection(),Logger('收藏')"></i>
			<i class="iconfont icon-share" @click="shareNews(),Logger('分享')"></i>
		</div>
		<comment-alert :current="'detail'" :requestData="requestData" :commentInput="commentInput" :commentPlace="$t('comment.saySomething')" @sendComment="sendComment"></comment-alert>
...
export default {
				...
				self.$nextTick(() => {
					...
					if (originalLink) {
						...
						originalLink.addEventListener('click', function(e) {
							if (self.$route.name === 'recommend') return;
							tooltipLeft.style.display = 'none';
							localStorage.detailTip = 'true';
							e.preventDefault();
	...
	methods: {
		...
		shareNews() {
            ...
            const sendObj = { body: location.href.replace('#/detail', '#/share'), title: self.detail.title, icon: `${self.imgBase}about_icon@3x.png`, type: 'share' }
